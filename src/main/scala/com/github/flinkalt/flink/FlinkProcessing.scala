...
object FlinkProcessing extends Processing[DataStream] {
  override def process1[S <: HList, A, B: TypeInfo](dataStream: DataStream[A])(f: StateTrans[S, A, Vector[B]])(implicit keyed: Keyed[A], si: StateInfo[S]): DataStream[B] = {
    ...
    dataStream
      .keyBy(keyed.fun)
      .process(new KeyedStateProcessing[keyed.K, S, A, B](si) {
        override def processElement(value: A, ctx: KeyedProcessFunction[keyed.K, A, B]#Context, out: Collector[B]): Unit = {
  ...
  override def process2[S <: HList, A, B1: TypeInfo, B2: TypeInfo](dataStream: DataStream[A])(f: StateTrans[S, A, (Vector[B1], Vector[B2])])(implicit keyed: Keyed[A], si: StateInfo[S]): (DataStream[B1], DataStream[B2]) = {
    ...
    val b1Stream = dataStream
      .keyBy(keyed.fun)
      .process(new KeyedStateProcessing[keyed.K, S, A, B1](si) {
        override def processElement(value: A, ctx: KeyedProcessFunction[keyed.K, A, B1]#Context, out: Collector[B1]): Unit = {
...
//noinspection ScalaRedundantCast
abstract class KeyedStateProcessing[K, S <: HList, A, Out](si: StateInfo[S]) extends KeyedProcessFunction[K, A, Out] {
  var stateDescriptors: Map[StateInfo[_], ValueStateDescriptor[_]] = _
